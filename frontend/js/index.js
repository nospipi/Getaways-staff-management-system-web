  ...
  return new Promise(function (resolveOuter) {
    getUserStatus(getName())
      ...
      .then(function (statusObject) {
        return new Promise(function (resolve) {
          if (
            statusObject.serverStatus == "unlogged" ||
            statusObject.localStatus == "unlogged"
          ) {
            handleLoginFlags("unlogged");
            resolve("authenticate");
            console.log(
              "status is unlogged,proceed to password authentication.."
            );
          } else {
            resolve("done");
        ...
        if (action == "authenticate") {
          sendPassword().then(function (response) {
            if (response == "correct") {
              ...
              console.log(
                "password is wrong,we re-run authentication sequence"
              );
      ...
      .then(function (response) {
        ...
        if (getName() == "Home") {
          necessaryData.staff.forEach((i) => {
            const tr_str = `<button class="btn btn-secondary" type="button" onclick="location.href='staff/index.php?staff=${i}'">${i}</button>`;
            $("#staff-links-wrap").append(tr_str);
          });
        } else {
          necessaryData.staff.forEach((i) => {
            const li = `<li><a class="dropdown-item show-by-staff">${i}</a></li>`;
        ...
        if (getName() !== "Home") {
          $.ajax({
            ...
            necessaryData.vehicles.forEach((i) => {
              const li = `<li><a class="dropdown-item show-by-vehicle">${i}</a></li>`;
...
function mutationObserve() {
  function balance() {
    $("#balance-table-wrap")
      .find(".fa-ellipsis-h")
      .toggleClass("rotate-ellipsis");
  }

  let balanceNode = document.querySelectorAll(
    "#balance-table-wrap .dropdown-menu"
  )[0];
  let balanceConfig = {
    attributes: true,
  ...
  let fuelNode = document.querySelectorAll(
    "#fuel-table-wrap .dropdown-menu"
  )[0];
  let fuelConfig = {
    attributes: true,
    attributeFilter: ["class"],
  ...
  let fleetNode = document.querySelectorAll(
    "#fleet-table-wrap .dropdown-menu"
  )[0];
  let fleetConfig = {
    attributes: true,
    attributeFilter: ["class"],
...
function getLocalLogStatus() {
  let status = localStorage.getItem(`${getName()}-logged`)
    ? "logged"
    : "unlogged";
...
function editStaff(name) {
  const newId =
    $("#update-staff-id").length > 0
      ? parseInt($("#update-staff-id").val().replace(/\D/g, ""))
      : 1;
  let oldName;
      ...
      success: function (response) {
        ...
        if (newName.length == 0 || newPassword.length == 0) {
          Swal.showValidationMessage(
            `<i class="fas fa-exclamation-circle" style="color: indianred"></i> Please fill all required fields !`
          );
          window.setTimeout(function () {
            $(".swal2-validation-message").fadeOut(500);
          }, 1000);
        } else {
          let allNames = $(".staff-name")
            .map(function () {
              return this.innerText;
            })
            .get(); //I'M GETTING AN ARRAY OF ALL STAFF NAMES..

          let allNamesFiltered = allNames.filter(
            (value) => value !== capitalize(oldName)
          ); // AND I'M FILTERING OUT THE OLD NAME OF THE RECORD...

          if (allNamesFiltered.includes(capitalize(newName)) == true) {
            //SO I CAN CHECK IF THERE IS A SAME NAME INSIDE THE LIST WITH THE NEW ONE..
            ...
            $.ajax({
              ...
              success: function (msg) {
                Swal.showValidationMessage(
                  `<i class="far fa-check-circle" style="color: green"></i> ${msg}`
                );
                window.setTimeout(function () {
...
$(document).on("click", ".edit-staff", function () {
  Swal.resetValidationMessage();
  Swal.showValidationMessage(
    `<i class="fas fa-exclamation-circle" style="color: indianred"></i>The user will be logged out !`
  );

  const id = parseInt(
    $(this).closest("tr").find(".staff-id").text().replace(/\D/g, "")
  );
...
$(document).on("click", ".staff-confirm-add", function () {
  ...
  if (
    $("#update-staff-name").val().length == 0 ||
    $("#update-staff-password").val().length == 0
  ) {
    Swal.showValidationMessage(
      `<i class="fas fa-exclamation-circle" style="color: indianred"></i> Please fill all required fields !`
    );
...
$(document).on("click", ".edit-vehicle", function () {
  ...
  const id = parseInt(
    $(this).closest("tr").find(".vehicle-id").text().replace(/\D/g, "")
  );
...
$(document).on("click", ".show-by-staff", function () {
      ...
      $(document).on("click", ".show-by-date", function () {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            let dateObj = {
              day: null,
              monthShort: getMonthString(
                parseInt($("#schedulePicker").val().replace(/.*-/, ""))
              ).substring(0, 3),
              monthLong: getMonthString(
                parseInt($("#schedulePicker").val().replace(/.*-/, ""))
              ),
  ...
  if (
    $(this).parents(".dropdown").next(".wrap").is("#fuel-wrap") ||
    $(this).parents(".wrap").is("#fuel-wrap")
  ) {
    loadFuel("driver", name, null);
  }

  if (
    $(this).parents(".dropdown").next(".wrap").is("#fleet-wrap") ||
    $(this).parents(".wrap").is("#fleet-wrap")
  ) {
        ...
        if (result.isConfirmed) {
          $.ajax({
            ...
            success: function (msg) {
              loadFuel(
                lastLoaded.fuel.db,
                lastLoaded.fuel.name,
                lastLoaded.fuel.plate
              ).then(() => {
                Swal.fire({
                  icon: "success",
                  title: msg,
        ...
        if (result.isConfirmed) {
          $.ajax({
            ...
            success: function (msg) {
              loadFleet(
                lastLoaded.fleet.db,
                lastLoaded.fleet.name,
                lastLoaded.fleet.plate,
                lastLoaded.fleet.editable
              ).then(() => {
                Swal.fire({
                  icon: "success",
                  title: msg,
...
$(document).on("click", ".schedule-table button", function () {
  ...
  if (getName() != "Admin") {
    html = `<div class="dropdown">
			<span style="display:none !important" class="this-parent-class ${$(this)
        .parents(".schedule-table")
        .attr("class")}"></span>
	        <span style="display:none !important" class="this-parent-name">${$(this)
            .parents(".schedule-table")
            .find(".driver")
            .text()}</span>
      ...
      if (response[0].comments.length > 0 || response[0].location.length > 0) {
        let comments =
          response[0].comments.length > 0
            ? `<div class="d-flex flex-column bg-light rounded border">
				<div class="text-dark p-1">Driver's comments :</div>
				<div class="fst-italic bg-white p-1 rounded-bottom">${response[0].comments}</div>
				</div>`
            : "";
        let location =
          response[0].location.length > 0
            ? `<button data-location="${response[0].location}" class="btn btn-primary text-nowrap fleet-location">Go to parking location</button>`
            : "";
        ...
        Swal.fire({
          icon: "error",
          title:
            "There is no vehicle location data or comments for this record",
          heightAuto: false,
        });
      }
    },
    ...
    const { value: text } = await Swal.fire({
      ...
      preConfirm: () => {
        if ($(".swal2-textarea").val().length > 0) {
          if (confirm("Submit request ?")) {
            return $.ajax({
              ...
              success: function (response) {
                if (response) {
                  Swal.showValidationMessage(
                    `<i class="far fa-check-circle" style="color: green"></i> ${response}`
                  );
                }
                loadAjax("requests");
          ...
          Swal.showValidationMessage(
            `<i class="fa fa-info-circle" style="color: indianred"></i>Please type your request/bug report`
          );
        }
      },
    });
...
function getCheckedBoxesAmount(selector) {
  let allCheckBoxesCheckedProps = [
    ...$(selector).closest(".wrap").find(".table .table-checkbox"),
  ].map((checkbox) => {
  ...
  if (allCheckBoxesCheckedProps.includes(false)) {
    $(selector)
      .closest(".wrap")
      .find(".table .table-select-all-checkbox")
      .prop("checked", false);
  } else {
    $(selector)
      .closest(".wrap")
  ...
  let checkedBoxes = [
    ...$(selector).parents(".wrap").find(".table-checkbox:checked"),
  ];
  ...
  if (checkedBoxes.length > 0) {
    ...
    $(selector)
      .closest(".wrap")
      .find(".btn-danger")
      .html(`<i class="m-0 far fa-trash-alt"></i>`);
  }
}
...
function addPickup() {
  const pickup = `<table class="table table-bordered plan-table">
<thead>
<tr>
...
function jobActions(action, id, classlist) {
  ...
  $(document).on("click", ".store-stop", function () {
    if (pickups[counter]) {
      Swal.showValidationMessage(
        `<i class="far fa-check-circle" style="color: green"></i> Pick-up #${
          counter + 1
        } is updated !!`
      );
      window.setTimeout(function () {
  ...
  $(document).on("click", ".delete-stop", function () {
    if (pickups.length !== 0) {
      //if array is not empty at click event

      if (confirm(`Delete pick-up #${counter + 1} ?`)) {
        Swal.showValidationMessage(
          `<i class="far fa-check-circle" style="color: red"></i> Pick-up #${
            counter + 1
          } is deleted !!`
        );
        window.setTimeout(function () {
        ...
        if (pickups.length === 0) {
          //if array becomes empty after splice
          newPickup();
          $("#pickup-plan-title").text(`Pick-up #${counter + 1}`);
        }
      }
    } else {
      // if array is empty at click event

      Swal.showValidationMessage(
        `<i class="fas fa-exclamation-circle" style="color: indianred"></i> No pick-ups stored !!`
      );
      window.setTimeout(function () {
        $(".swal2-validation-message").fadeOut(2000);
      }, 1000);
  ...
  function swapPickups(direction) {
    ...
    function swap(counter) {
      let rows_html = pickups[counter].guests
        .map((obj, index) => {
  ...
  function handleLocationMarkers() {
    ...
    $(".pickup-plan-visual-counter i").length > 0
      ? $("#visual-counter-wrap").show()
      : $("#visual-counter-wrap").hide();
  }
        ...
        if (steps[currentStep] == 1) {
          var result = await swalQueueStep.fire({
            ...
            didOpen: () => {
              ...
              document
                .querySelectorAll("#driver-select option")
                .forEach((i) => {
                  if ($(i).text() == driver) {
          ...
          var result = await swalQueueStep.fire({
            ...
            didOpen: () => {
              ...
              document
                .querySelectorAll("#escort-select option")
                .forEach((i) => {
                  if ($(i).text() == escort) {
          ...
          var result = await swalQueueStep.fire({
            ...
            didOpen: () => {
              document
                .querySelectorAll("#activity-select option")
                .forEach((i) => {
                  if ($(i).text() == activity) {
          ...
          var result = await swalQueueStep.fire({
            ...
            didOpen: () => {
              ...
              document
                .querySelectorAll("#vehicle-select option")
                .forEach((i) => {
                  if ($(i).text() == vehicle) {
          ...
          var result = await swalQueueStep.fire({
            ...
            didOpen: () => {
              ...
              let rows_html =
                pickups.length > 0
                  ? pickups[counter].guests
                      .map((obj, index) => {
                        let details =
                          obj.details.length > 0 ? obj.details : "-";
                        return `<tr class="plan-row">
							<td class="plan-row-id">${"#" + (index + 1)}</td>
							<td contenteditable="true" style="text-transform: capitalize">${obj.name}</td>
							<td contenteditable="true">${obj.pax}</td>
              ...
              $("#pickup-plan-title").text(`Pick-up #${counter + 1}`);
              if (pickups.length == 0) {
                // if we dont have anything stored in the array "pickups" ...
                $(".pickup-plan-tbody").append(addPlanRows(1)); // ... we just add one row
              } else {
                // ... otherwise we add the content of the first "pickups" array object -->
                $(".pickup-plan-mp").text(pickups[counter].meeting_point);
                $(".pickup-plan-time").text(pickups[counter].time);
  ...
  function addNewCall() {
    $.ajax({
      ...
      success: function (response) {
        [lastLoaded.schedule.name, lastLoaded.schedule.pending] = [
          driver,
          true,
        ];

        loadSchedule(
          lastLoaded.schedule.name,
          lastLoaded.schedule.date,
          lastLoaded.schedule.pending
        ).then(() => {
          saveLog(
            "New activity to " + driver,
            moment().format("DD/MM/YYYY HH:mm"),
            ""
          );
          ...
          if (getName() === "Admin") {
            var result = await swalQueueStep.fire({
              ...
              didOpen: () => {
                ...
                document
                  .querySelectorAll("#staff-select option")
                  .forEach((i) => {
                    if ($(i).text() == staff) {
    ...
    $.ajax({
      ...
      success: function (msg) {
        loadBalance(staff).then(() => {
          saveLog(
            "New transaction for " + capitalize(staff),
            moment().format("DD/MM/YYYY HH:mm"),
            "balance"
          );
      ...
      for (currentStep = 0; currentStep < steps.length; ) {
        if (steps[currentStep] == 1) {
          ...
          var result = await swalQueueStep.fire({
            ...
            didOpen: () => {
              document
                .querySelectorAll("#payment-select option")
                .forEach((i) => {
                  if ($(i).text() == payment) {
    ...
    $.ajax({
      ...
      success: function (msg) {
        loadFuel(
          ...
          saveLog(
            "New fuel report for " + capitalize(staff),
            moment().format("DD/MM/YYYY HH:mm"),
            "fuel"
          );
...
function createScheduleTables(arr, member) {
  return arr
    .map((obj) => {
      //MAP RETURNS A NEW ARRAY FOR EVERY ITERATION
      ...
      let statusIcon =
        obj.class == "jobdone"
          ...
          : obj.class == "cancelled"
          ? `<i class="fas fa-exclamation align-middle"></i>`
          : "";
      let commentIcon = obj.remarks
        ? `<i class="fas fa-comment align-middle"></i>`
        : "";
      let iconColumn =
        statusIcon == "" && commentIcon == ""
          ? ""
          : `<div class="col border border-bottom-0 border-end-0 border-1 p-1 d-flex flex-row justify-content-center align-items-center gap-2">
		${statusIcon} ${commentIcon}
		</div>`;
...
function createClickedScheduleTable(state, obj) {
  let details_html =
    obj.details !== null && obj.details.length > 0
      ? `
      ...
      ? `<div class="row bg-secondary text-light">
        ...
        pickups
          .map((obj) => {
			...
			<td class="schedule-gps" rowspan="${
        obj.guests.length + 1
      }"><i class="fas fa-location-arrow text-success"></i></td>
			<td class="capitalize border-0" rowspan="${obj.guests.length + 1}">${
                obj.meeting_point
              }</td>
...
function loadSchedule(name, date, pending) {
  ...
  return (myPromise = new Promise(function (resolve) {
    let headerMonth =
      name == "Today" ? "" : pending == true ? "pending" : date.monthLong;

    [
      lastLoaded.schedule.name,
      lastLoaded.schedule.date.day,
      lastLoaded.schedule.date.monthShort,
      lastLoaded.schedule.date.monthLong,
      lastLoaded.schedule.date.year,
      lastLoaded.schedule.pending,
    ] = [name, date.day, date.monthShort, date.monthLong, date.year, pending];
    ...
    if (action == "success") {
      return (myPromise = new Promise((resolve) => {
        ...
        for (var i = 0; i < len; i++) {
          ...
          var tr_str = `<tr class="${dbclass}-row">
...
function loadFuel(db, name, plate) {
  return (myPromise = new Promise(function (resolveOuter) {
    let inner = new Promise((resolve) => {
      ...
      callFuelDatabase(db, name, plate)
        .then((success) => {
...
function loadFleet(db, name, plate, editable) {
  return (myPromise = new Promise(function (resolveOuter) {
    let inner = new Promise((resolve) => {
      ...
      callFleetDatabase(db, name, plate)
        .then((success) => {
...
<td class="table-buttons"><div class="table-button text-light">Edit</div><input class="table-checkbox" type="checkbox" style="display:none;width: 1.9em;height: 1.9em"></td>
        ...
        $(
          "#fleet-table-wrap table thead tr:nth-child(1) th:nth-child(2)"
        ).show();
        $(
          "#fleet-table-wrap table thead tr:nth-child(2) th:nth-child(8)"
        ).show();
      } else {
        tr_str = `<tr class="all-fleet">
<td class="fleet-gps p-1"><i class="fas fa-question-circle text-success" style="font-size:1.5em"></i></td>
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        for (var i = 0; i < len; i++) {
          ...
          var tr_str =
            "<tr>" +
            '<td class="log-user p-1 border-bottom-0">' +
            user +
            "</td>" +
            '<td class="log-action p-1 border-bottom-0">' +
            action +
            "</td>" +
            '<td class="log-date p-1 border-bottom-0">' +
            date +
            "</td>" +
            "</tr>";
  ...
  return (myPromise = new Promise(function (resolve) {
    if (table == "schedule") {
      ...
      const worksheet = workbook.addWorksheet(
        `${scheduleData.name} ${scheduleData.month} ${scheduleData.year} schedule`
      );
    ...
    if (table == "balance") {
      ...
      [1, 2, 3, 4, 5].map(
        (x) =>
          (worksheet.getRow(1).getCell(x).border = {
            top: {
              style: "thin",
              color: {
                argb: "8c8c8c",
      ...
      workbook.xlsx.writeBuffer().then((data) => {
        ...
        let filename = `${name}-balance_${moment().format(
          "DD-MMM-YYYY_HH:mm"
        )}`;
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        for (var i = 0; i < len; i++) {
          ...
          let tr_str = `<tr class="table-light">
